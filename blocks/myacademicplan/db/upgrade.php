...
/**
 * This file keeps track of upgrades to the myacademicplan block
 *
 * @since 3.8
...
/**
 * Upgrade code for the myacademicplan block.
 *
 * @param int $oldversion
 */
function xmldb_block_myacademicplan_upgrade($oldversion) {
    ...
    if ($oldversion < 2019091800) {
        ...
        upgrade_block_savepoint(true, 2019091800, 'myacademicplan', false);
    }
    ...
    if ($oldversion < 2019111801) {
        ...
        $dbval = $DB->get_field('config_plugins', 'value', ['plugin' => 'block_myacademicplan', 'name' => 'displaygroupingstarred']);
        if ($dbval !== false) {
            set_config('displaygroupingfavourites', $dbval, 'block_myacademicplan');
            unset_config('displaygroupingstarred', 'block_myacademicplan');
        }

        if (isset($CFG->forced_plugin_settings['block_myacademicplan']['displaygroupingstarred'])) {
            // Check to see if the starred setting is defined in the config file. Display a warning if so.
            $warn = 'Setting block_myacademicplan->displaygroupingstarred has been renamed '.
                    'to block_myacademicplan->displaygroupingfavourites. Old setting present in config.php.';
            echo $OUTPUT->notification($warn, 'notifyproblem');
        }

        upgrade_block_savepoint(true, 2019111801, 'myacademicplan', false);
    }
