 ...
 */
namespace block_myacademicplan\output;
...
require_once($CFG->dirroot . '/blocks/block_myacademicplan/lib.php');
...
class main implements renderable, templatable {
     ...
     *
     * @var string String matching the grouping constants defined in MYACADEMICPLAN/lib.php
     */
    private $grouping;
     ...
     *
     * @var string String matching the sort constants defined in MYACADEMICPLAN/lib.php
     */
    private $sort;
     ...
     *
     * @var string String matching the view/display constants defined in MYACADEMICPLAN/lib.php
     */
    private $view;
     ...
     *
     * @var string String matching the paging constants defined in MYACADEMICPLAN/lib.php
     */
    private $paging;
    ...
    /**
     * Store the configuration values for the MYACADEMICPLAN block.
     *
     * @var array Array of available layouts matching view/display constants defined in MYACADEMICPLAN/lib.php
     */
    private $layouts;
    ...
    public function __construct($grouping, $sort, $view, $paging, $customfieldvalue = null) {
        global $CFG;
        // Get plugin config.
        $config = get_config('block_myacademicplan');
        //print_object($config);
        // Cree el nombre de la opción de agrupación del curso para verificar si la agrupación dada se habilita después.
        $groupingconfigname = 'displaygrouping'.$grouping;
        // Verifique el grupo dado y recuérdelo si está habilitado.
        if ($grouping && $config->$groupingconfigname == true) {
            $this->grouping = $grouping;

            // De lo contrario, recurra a otra agrupación en un orden razonable.
            // Esto se hace para evitar problemas técnicos únicos de la interfaz de usuario en el caso de que un usuario haya elegido previamente una opción de agrupación que el administrador desactivó mientras tanto.
        } else {
            $this->grouping = $this->get_fallback_grouping($config);
        }
        unset ($groupingconfigname);

        // Recuerde qué valor de campo personalizado estábamos usando, si lo agrupamos por campo personalizado.
        $this->customfieldvalue = $customfieldvalue;

        // Verifique y recuerde la clasificación dada.
        if ($sort) {
            $this->sort = $sort;
        } else if ($CFG->courselistshortnames) {
            $this->sort = BLOCK_MYACADEMICPLAN_SORTING_SHORTNAME;
        } else {
            $this->sort = BLOCK_MYACADEMICPLAN_SORTING_TITLE;
        }
        // In case sorting remembered is shortname and display extended course names not checked,
        // deberíamos revertir la clasificación al título.
        if (!$CFG->courselistshortnames && $sort == BLOCK_MYACADEMICPLAN_SORTING_SHORTNAME) {
            $this->sort = BLOCK_MYACADEMICPLAN_SORTING_TITLE;
        }

        // Verifique y recuerde la vista dada.
        $this->view = $view ? $view : BLOCK_MYACADEMICPLAN_VIEW_CARD;

        // Verifique y recuerde el tamaño de página dado.
        if ($paging == BLOCK_MYACADEMICPLAN_PAGING_ALL) {
            $this->paging = BLOCK_MYACADEMICPLAN_PAGING_ALL;
        } else {
            $this->paging = $paging ? $paging : BLOCK_MYACADEMICPLAN_PAGING_12;
        }

        // Verifique y recuerde si las categorías del curso deben mostrarse o no.
        if (!$config->displaycategories) {
            $this->displaycategories = BLOCK_MYACADEMICPLAN_DISPLAY_CATEGORIES_OFF;
        } else {
            $this->displaycategories = BLOCK_MYACADEMICPLAN_DISPLAY_CATEGORIES_ON;
        }

        // Obtenga y recuerde los diseños disponibles.
    ...
    private function get_fallback_grouping($config) {
        if ($config->displaygroupingall == true) {
            return BLOCK_MYACADEMICPLAN_GROUPING_ALL;
        }
        if ($config->displaygroupingallincludinghidden == true) {
            return BLOCK_MYACADEMICPLAN_GROUPING_ALLINCLUDINGHIDDEN;
        }
        if ($config->displaygroupinginprogress == true) {
            return BLOCK_MYACADEMICPLAN_GROUPING_INPROGRESS;
        }
        if ($config->displaygroupingfuture == true) {
            return BLOCK_MYACADEMICPLAN_GROUPING_FUTURE;
        }
        if ($config->displaygroupingpast == true) {
            return BLOCK_MYACADEMICPLAN_GROUPING_PAST;
        }
        if ($config->displaygroupingfavourites == true) {
            return BLOCK_MYACADEMICPLAN_GROUPING_FAVOURITES;
        }
        if ($config->displaygroupinghidden == true) {
            return BLOCK_MYACADEMICPLAN_GROUPING_HIDDEN;
        }
        if ($config->displaygroupingcustomfield == true) {
            return BLOCK_MYACADEMICPLAN_GROUPING_CUSTOMFIELD;
        }
    ...
    public function set_available_layouts() {
        ...
        if ($config = get_config('block_myacademicplan', 'layouts')) {
            //print_object(get_config('block_MYACADEMICPLAN', 'layouts'));
            $this->layouts = explode(',', $config);
        } else {
            $this->layouts = array(BLOCK_MYACADEMICPLAN_VIEW_CARD);
        }
    }
    ...
    public function format_layout_for_export($layoutname) {
        ...
        $layout->name = get_string($layoutname, 'block_myacademicplan');
        $layout->active = $this->view == $layoutname ? true : false;
        $layout->arialabel = get_string('aria:' . $layoutname, 'block_myacademicplan');
    ...
    public function get_customfield_values_for_export() {
        ...
        $customfieldactive = ($this->grouping === BLOCK_MYACADEMICPLAN_GROUPING_CUSTOMFIELD);
        $ret = [];
    ...
    public function export_for_template(renderer_base $output) {
        ...
        $nocoursesurl = $output->image_url('courses', 'block_myacademicplan')->out();

        $customfieldvalues = $this->get_customfield_values_for_export();
        $selectedcustomfield = '';
        if ($this->grouping == BLOCK_MYACADEMICPLAN_GROUPING_CUSTOMFIELD) {
            ...
            if (!$selectedcustomfield) {
                $this->grouping = $this->get_fallback_grouping(get_config('block_myacademicplan'));
                if ($this->grouping == BLOCK_MYACADEMICPLAN_GROUPING_CUSTOMFIELD) {
        ...
        $sort = '';
        if ($this->sort == BLOCK_MYACADEMICPLAN_SORTING_SHORTNAME) {
            $sort = 'shortname';
        } else {
            $sort = $this->sort == BLOCK_MYACADEMICPLAN_SORTING_TITLE ? 'fullname' : 'ul.timeaccess desc';
        }
